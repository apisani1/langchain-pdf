...
def singlestore_vector_store_builder(
    ...
    return SingleStore.connect(
        index_name=os.getenv("SINGLESTOREDB_TABLE", "docs")
        + "_"
        + splitter_name
        + "-"
        + embedding_name,
        host=os.getenv("SINGLESTOREDB_HOST"),
        database=os.getenv("SINGLESTOREDB_DATABASE", "mydb"),
        embedding=embedding,
        id_key="chunk_id",
        **kwargs,
    )
...
def singlestore_retriever_builder(
    ...
    return chat_config.vector_store_map[splitter_name]["singlestore"][
        embedding_name
